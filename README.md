...
> [!WARNING]
> 只用于你自己的账号、设备和已获授权的数据。不要用于未授权访问，也不要公开上传数据库、key、JSONL、聊天语料或日志。
...
先下载配套构建的[ SQLCipher CLI ](https://github.com/ShintoKosei/sqlcipher-windows-builds/releases/latest)并解压到本仓库的 `tools\sqlcipher`目录。
...
请先在PC端与Android上配置好frida环境，需要root。
...
默认导出到手机：`/storage/emulated/0/Download/qq_nt_msg_plaintext.db
`
...
- [sqlcipher/sqlcipher](https://github.com/sqlcipher/sqlcipher)：本项目配套的 Windows SQLCipher CLI 构建原仓库。
